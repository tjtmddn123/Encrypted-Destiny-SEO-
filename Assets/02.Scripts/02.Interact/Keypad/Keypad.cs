...
namespace NavKeypad
{
    public class Keypad : MonoBehaviour
    {
        ...
        [SerializeField] private int keypadCombo = 12345; //비밀번호 설정

        public UnityEvent OnAccessGranted => onAccessGranted;
        public UnityEvent OnAccessDenied => onAccessDenied;

        [Header("Settings")]
        [SerializeField] private string accessGrantedText = "Granted"; //성공 시 텍스트
        [SerializeField] private string accessDeniedText = "Denied";  //실패 시  텍스트

        [Header("Visuals")]
        [SerializeField] private float displayResultTime = 1f; // 결과화면이 떠있는 시간
        [Range(0, 5)]
        ...
        public void CheckCombo()
        {
            if (int.TryParse(currentInput, out var currentKombo))
            {
                ...
                if (!displayingResult)
                {
                    StartCoroutine(DisplayResultCorutine(granted));
                }
            }
            else
            {
        ...
        private IEnumerator DisplayResultCorutine(bool granted)
        {
